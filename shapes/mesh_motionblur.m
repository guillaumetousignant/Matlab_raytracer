classdef mesh_motionblur < shape
...
methods
    function obj = mesh_motionblur(geometry, material, transform)
        ...
        if isstruct(material)
            for i = 1:obj.ntris
                obj.triangles{i} = triangle_mesh_motionblur(material.(obj.geom.mat{i, 1}), obj.geom, i, obj.transformation);
            end
        else
            for i = 1:obj.ntris
                obj.triangles{i} = triangle_mesh_motionblur(material, obj.geom, i, obj.transformation);
            end
        end
    end
